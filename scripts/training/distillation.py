...
class DistillationTrainer(Trainer):
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        """
        Override compute_loss to include both standard training and distillation components
        while maintaining core Trainer functionality.
        """
        if self.distill_loss_weight ==0 and self.encoder_loss_weight == 0 and self.decoder_loss_weight == 0:
        ...
        teacher_encoder_hidden = F.normalize(teacher_encoder_hidden, p=2, dim=-1)
        student_decoder_hidden = F.normalize(student_decoder_hidden, p=2, dim=-1)
        ...
        )/student_encoder_hidden.size(0)
        
        decoder_loss = F.mse_loss(
            student_decoder_hidden,
            teacher_decoder_hidden,reduction='sum'
        )/student_decoder_hidden.size(0)
        ...
        self.losses["task_loss"] += task_loss.item()
        self.losses["distill_loss"] += distill_loss.item()
        self.losses["encoder_loss"] += encoder_loss.item()
        self.losses["decoder_loss"] += decoder_loss.item()

        if self.args.logging_steps > 1 and self.state.global_step % self.args.logging_steps == 0:
            self.log({
                "task_loss": self.losses["task_loss"] / self.args.logging_steps,
                "encoder_loss": self.losses["encoder_loss"] / self.args.logging_steps,
                "decoder_loss": self.losses["decoder_loss"] / self.args.logging_steps,
                "distill_loss": self.losses["distill_loss"] / self.args.logging_steps,
            })
            self.losses = {k: 0 for k,v in self.losses.items()}
    ...
    def create_optimizer(self):
        ...
        optimizer_grouped_parameters = [
            {
                "params": [
                    ...
                    p for p in self.encoder_projection.parameters() if p.requires_grad
                ] + [
                    p for p in self.decoder_projection.parameters() if p.requires_grad
                ],
        ...
        }.get(self.args.optim, torch.optim.AdamW) 
...
def main(
    ...
    teacher_id: str = "amazon/chronos-t5-small",
    task_loss_weight: float = 0.5,
    distill_loss_weight: float = 0.5,
    encoder_loss_weight: float = 0.5,
    ...
    student_hidden_size = hidden_dims[model_id.split("-")[-1]]
    teacher_hidden_size = hidden_dims[teacher_id.split("-")[-1]]
